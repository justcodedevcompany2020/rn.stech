...
export default class App extends Component {
    ...
    getNotifications = async () => {
        ...
        try {
            fetch(`${APP_URL}/getMyNotoficationRoleId2?page=1`, {
                method: 'GET',
                headers: {
                ...
                if (response.data.hasOwnProperty('message')) {
                    if (response.data.message == 'no notification') {
                       await this.setState({
                            no_notification: true,
                            no_notification_text: 'Уведомлений нет',
                ...
                if (response.data.hasOwnProperty('notification_list')) {
                    if (response.data.notification_list.hasOwnProperty('data')) {
                        if (response.data.notification_list.data.length > 0) {
                            this.setState({
                                notifications: response.data.notification_list.data
                            })
                        } else {
                            this.setState({
                                notifications: [],
                    ...
                    if (response.data.hasOwnProperty('notification_list')) {
                            if (response.data.notification_list.hasOwnProperty('next_page_url')) {
                                if (response.data.notification_list.next_page_url) {
                                    this.setState({
                                        next_page_url: response.data.notification_list.next_page_url,
                                    })
                                } else {
                                    this.setState({
                                        loading_notifications: false,
    ...
    loadMoreNotifications = async () => {
        ...
        let userToken = await AsyncStorage.getItem('userToken');
        let AuthStr = 'Bearer ' + userToken;
        let {next_page_url, notifications} = this.state;
        ...
        try {
            fetch(next_page_url, {
                ...
                let new_notifications = response.data.notification_list.data;

                new_notifications.forEach(function(item){
    ...
    getTime = (date ) =>
    {
        // let startTime = moment('2023-01-30T14:22:11.000000Z')
        ...
        let result = '';
        if (second < 60) {
            result = `${second.toFixed(0)}  секунд назад`;
        } else if(second >= 60 && minutes < 60) {
            result = `${minutes.toFixed(0)} минут назад`;
        } else if(minutes >= 60 && hours < 24) {
            result = `${hours.toFixed(0)} часов назад`;
        } else if(hours >= 24) {
            result = `${day.toFixed(0)} дней назад`;
        }
    ...
    redirectToExecutorChatSinglePage = () => {
    ...
    deleteNotification = async () => {
        let userToken = await AsyncStorage.getItem('userToken');
        let AuthStr = 'Bearer ' + userToken;
        let notification_id = this.state.notification_id

        try {
            fetch(`${APP_URL}/delete_notification`, {
                method: 'POST',
                headers: {
                    'Authorization': AuthStr,
                    'Accept': 'application/json',
    ...
    openDeleteNotificationPopup = (id) => {
        this.setState({
    ...
    render() {
        if (this.state.deleteNotificationPopup) {
            return (
                <ImageBackground resizeMode="cover" source={require('../../../assets/images/popup_img.png')} style={[styles.orderModerationPopup,{width: '100%', height: windowHeight,  zIndex: 99999999, }]}>
                    <View style={styles.order_moderation_popup_wrapper}>
                        <TouchableOpacity style={styles.single_product_popup_close_btn} onPress={() => {this.setState({deleteNotificationPopup: false})}}>
                            <Svg
                                width={30}
                                height={30}
                                viewBox="0 0 30 30"
                        ...
                        <TouchableOpacity style={styles.go_to_single_product_btn} onPress={() => this.deleteNotification()}>
                            <Text style={styles.go_to_single_product_btn_text}>Подтвердить</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.cancel_delete_tender_btn} onPress={() => {this.setState({deleteNotificationPopup: false})}}>
                            <Text style={styles.cancel_delete_tender_btn_text}>Отменить</Text>
                        </TouchableOpacity>
        ...
        return (
            <SafeAreaView  edges={['right', 'left', 'top']} style={styles.container} >
                ...
                <View style={styles.client_notifications_header}>
                    <TouchableOpacity  style={styles.client_notifications_header_back_btn} onPress={() => this.props.navigation.goBack()}>
                    ...
                    <Text style={styles.client_notifications_header_title}>Уведомления</Text>
                </View>

                {this.state.no_notification &&
                    <View style={styles.no_notification_error}>
                        <Text style={styles.no_notification_error_text}>{this.state.no_notification_text}</Text>
                    </View>
                }
                ...
                {this.state.notifications.length > 0 &&
                    <View style={{flex:1, width: '100%'}}>
                        ...
                        <FlatList
                            ...
                            renderItem={({item}) => (
                                <View   style={styles.notification_item}>
                                    <View style={styles.notification_img_info_wrapper}>
                                        <View style={styles.notification_img}>
                                            <Image style={styles.notification_img_child}  source={{uri: this.state.image_path + item?.author_notification?.photo}} />
                                        </View>
                                    </View>

                                    <Text style={styles.notification_info1}>{item.message}</Text>

                                    <View style={styles.notification_delete_btn_info_box}>
                                        <Text style={styles.notification_item_hour_info}>{this.getTime(item.created_at)}</Text>
                                        <TouchableOpacity onPress={() => {this.openDeleteNotificationPopup(item.id)}}>
                                            <Svg
                                                width={20}
                                                height={20}
                                                viewBox="0 0 15 15"
                                            ...
                                            >
                                                <Path
                                                    d="M13.125 3.737a63.563 63.563 0 00-6.263-.312c-1.237 0-2.474.062-3.712.187l-1.275.125M5.313 3.106l.137-.818c.1-.594.175-1.038 1.231-1.038H8.32c1.056 0 1.137.469 1.231 1.044l.137.812M11.781 5.713l-.406 6.294c-.069.98-.125 1.743-1.869 1.743H5.494c-1.744 0-1.8-.762-1.869-1.743l-.406-6.294M6.456 10.313h2.081M5.938 7.813h3.125"
                                                    Подтвердите
                                                    удаление тендера       stroke="#292D32"
                                                    strokeLinecap="round"
                        ...
                        <View style={{justifyContent: 'center', alignItems: 'center', alignSelf: 'center', width: '100%',  height: 40, position:'absolute', bottom: 60}}>
                ...
                <View style={styles.footer}>
                    <TouchableOpacity style={[styles.footer_btn, {position: 'relative'}]}>
                        ...
                        <Svg width={35} height={38} viewBox="0 0 35 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <G clipPath="url(#clip0_486_3554)" fill="#FF6600">
                            ...
                            <Defs>
                                <ClipPath id="clip0_486_3554">
                                    <Path fill="#FF6600" transform="translate(0 .5)" d="M0 0H35V35H0z" />
                                </ClipPath>
                            </Defs>
                        </Svg>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    client_notifications_header_title: {
        fontWeight: '700',
        fontSize: 17,
        color: '#333333',
        textAlign: 'center',
    ...
    client_notifications_header_back_btn: {
        position: 'absolute',
        left: 15,
        top: 20,
    },
    notification_items_main_wrapper: {
        width: '100%',
        flex: 1,
        // paddingTop: 20,
        paddingBottom: 100,
    ...
    no_notification_error_text: {
        color: '#333333',
        fontWeight: '700',
        fontSize: 20,
    },
    notification_delete_btn_info_box: {
        flexDirection: 'row',
        alignItems: 'center'
    },
