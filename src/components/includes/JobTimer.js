...
export default class App extends Component {
            ...
            nextAppState => {
                if (
                    ...
                    if (this.state.start_time != '') {
                        this.startTimer(false, this.state.start_time)
                           ...
                           if (response.message == 'You Have Active Job') {
                                if (!response.data.start_job && !response.data.end_job) {
                                   ...
                                   await this.setState({
                                        ...
                                        start_job: false,
                                        confirm_job_result: false,
                                        start_time: response.data.start_job

                                    })

                                    this.startTimer(false, response.data.start_job)
                                } else if (response.data.start_job && response.data.end_job && response.data.status == '2') {

                                    let total_time = response.time.split(':');
                                    ...
                                    let result_time = total_time.join(':');

                                   await this.setState({
                                        end_job: false,
                                        start_job: false,
                                        job_result_price: response.price,
                                        timer_value: result_time,
                    ...
                    fetch(`${APP_URL}/endJob`, {
                        ...
                        if (response.hasOwnProperty('status')) {
                            if (response.status === true) {
                                ...
                                let total_time = response?.data?.time.split(':');
    ...
    openChat = () => {
        ...
        if (tender_id) {
            this.props.navigation.navigate("ExecutorTenderChatSinglePage", {
                ...
                redirect_from_type: 'from job timer'
    ...
    startTimer( from_zero = true, start_job = null )
    {
        let seconds
        let minutes
        let hours
        ...
        if(from_zero)
        {
            ...
            let start_time = moment(start_job)
            let current_date_time   = moment()

            let duration = moment.duration(current_date_time.diff(start_time));

            // let hours   = duration.asHours();
            // let minutes = duration.asMinutes();
            let second  = duration.asSeconds();
            hours = this.formatTime(second).hours
            minutes = this.formatTime(second).minutes
            seconds = this.formatTime(second).seconds.toFixed(0)
        ...
        this.interval = setInterval(() => {
            ...
            let timeString = (hours ? (hours > 9 ? hours : "0" + hours) : "00") + ":" +
                (minutes ? (minutes > 9 ? minutes : "0" + minutes) : "00") + ":" +
                (seconds > 9 ? seconds : "0" + seconds);


            _this.setState({
                timer_value: timeString
            })
              ...
              <ImageBackground resizeMode="cover" source={require('../../assets/images/timer_img.png')} style={[{width: '100%', height: '100%',  zIndex: 99999999, }]}>
                    ...
                    >
                        ...
                        {this.state.start_job &&
                             <Text style={styles.timer_main_title}>Время работы</Text>
                        }
                        ...
                        <LinearGradient
                            ...
                            style={styles.timer_box}
                        >
                            <View style={styles.timer_box_child}>
                                {/*<Text style={styles.timer_box_title}>{this.state.job_result.time}</Text>*/}
                                <Text style={styles.timer_box_title}>{this.printTimerValue()}</Text>
                            </View>
