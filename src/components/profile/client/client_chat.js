...
export default class App extends Component {
    ...
    redirectToClientChatSinglePage = async (chat_item) => {

        let receiver_id = chat_item.receiver_id;
        let user_name = chat_item.user_name;
        let user_image = chat_item.user_image;
    ...
    redirectToClientTendersChat = async () => {
        clearInterval(this.chat_interval)
        this.props.navigation.navigate("ClientTendersChat");
    }

    timeout = () => {
       this.timeout2 =  setTimeout(this.intervalFunc, 3000); // Here
    }
        ...
        try {
            fetch(`${APP_URL}/ChatUser`, {
                ...
                if (response.success === true) {
                    let slujba_podjerki = [];
                    let users_data = response.userschatdata;

                    for (let i = 0; i < users_data.length ; i++) {
                        if (users_data[i].user_name == 'Cлужба поддержки') {
                                slujba_podjerki = users_data[i]
                        }
                    ...
                    let key = 'user_name';
                    let value = 'Cлужба поддержки';

                    users_data = users_data.filter(object => object[key] !== value);



                    let sortedData = users_data.sort((a, b) => a.date > b.date ? -1 : 1)

                    this.setState({
                        userschatdata: sortedData,
                        support_service: slujba_podjerki
                    })
                }
    ...
    render() {
        ...
        let support_service = this.state.support_service
        return (
                    ...
                    >
                        <View style={styles.client_chat_header}>
                            ...
                            <View style={styles.client_chat_buttons_wrapper}>
                                ...
                                <TouchableOpacity style={[styles.client_chat_button ]} onPress={() => {this.redirectToClientTendersChat()}}>
                ...
                >
                    ...
                    {support_service !== null &&
                                    ...
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {this.redirectToClientChatSinglePage(support_service)}}
                                    >

                                        <View style={styles.client_chat_item_img_info_wrapper}>
                                            <View style={[styles.client_chat_item_img, {position: 'relative', top: 6}]}>
                                                <Image style={{width: 60, height: 50, resizeMode: 'cover'}}
                                                       source={require('../../../assets/images/chat_admin.png')}/>

                                            </View>
                                            <View style={styles.client_chat_item_info_box}>
                                                <Text style={[styles.client_chat_item_info1, {marginBottom: 6}]}>{support_service.user_name}</Text>
                                                ...
                                                {support_service.file &&
                                                    <View style={{flexDirection: 'row', width: '100%', alignItems: 'center'}}>
                                                        ...
                                                        <Text style={styles.client_chat_item_info3} >Фото</Text>
                                                    </View>
                                                    // <Image style={{width: 35, height: 33}} source={{uri: this.state.image_path + support_service.file}}/>
                                                }

                                            </View>
                                        </View>
                                        <View style={styles.client_chat_item_notification_hour_info_wrapper}>
                                            {support_service.review > 0 &&
                                            <View style={styles.client_chat_item_notification_box}>
                                                <Text style={styles.client_chat_item_notification_info}>{support_service.review}</Text>
                                            </View>
                                            }


                                            <Text style={styles.client_chat_item_hour_info}>{this.getTime(support_service.tender_data)}</Text>
                                        </View>
                                    </TouchableOpacity>
                    ...
                    {this.state.searchInput.length > 0
                                    ...
                                    {this.state.searchResult.map((search_item, index) => {
                                        ...
                                        return (
                                            ...
                                            <TouchableOpacity key={index}
                                                              style={styles.client_chat_item}
                                                              onPress={() => {this.redirectToClientChatSinglePage(search_item)}}
                                            >

                                                <View style={styles.client_chat_item_img_info_wrapper}>
                                                    <View style={styles.client_chat_item_img}>
                                                        {search_item.receiver_id == 1
                                                            ?
                                                            <Image style={{width: 60, height: 47, resizeMode: 'cover'}}
                                                                   source={require('../../../assets/images/chat_admin.png')}/>
                                                            :
                                                            <Image style={[styles.client_chat_item_img_child, { borderRadius: 100,}]} source={{uri: this.state.image_path + search_item.user_image}}/>
                                                        }
                                                    ...
                                                    <View style={styles.client_chat_item_info_box}>
                                                        ...
                                                        {search_item.file &&
                                                            ...
                                                            // <Image style={{width: 35, height: 33}} source={{uri: this.state.image_path + search_item.file}}/>
                                                        }
                        ...
                        <View style={styles.chat_lists_main_wrapper}>
                            {this.state.userschatdata.length > 0 ?
                                    ...
                                    {this.state.userschatdata.map((chat_item, index) => {
                                        ...
                                        return (
                                            <View key={index} style={{width: '100%'}}>
                                                {chat_item.user_name !== 'Cлужба поддержки' &&
                                                    <TouchableOpacity
                                                                      style={styles.client_chat_item}
                                                                      onPress={() => {this.redirectToClientChatSinglePage(chat_item)}}
                                                    >

                                                        <View style={styles.client_chat_item_img_info_wrapper}>
                                                            <View style={styles.client_chat_item_img}>
                                                                {chat_item.receiver_id == 1
                                                                    ?
                                                                    <Image style={{width: 60, height: 47, resizeMode: 'cover'}}
                                                                           source={require('../../../assets/images/chat_admin.png')}/>
                                                                    :
                                                                    <Image style={[styles.client_chat_item_img_child, { borderRadius: 100,}]} source={{uri: this.state.image_path + chat_item.user_image}}/>
                                                                }
                                                            ...
                                                            <View style={styles.client_chat_item_info_box}>
                                                                ...
                                                                {chat_item.file &&
                                                                    ...
                                                                    // <Image style={{width: 35, height: 33}} source={{uri: this.state.image_path + chat_item.file}}/>
                                                                }
                                ...
                                :
                                <Text style={styles.not_found_text}>Список пуст</Text>
                            }
