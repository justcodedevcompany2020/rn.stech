...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            selectedItemCityID: null,
            selectedItemRegionID: null,
            regions_data: [],
            regions_data_for_dropdown: [],


            cities_data: [],
            cities_data_for_dropdown: [],
            ...
            editNewPhoneNumber_error: false,
            editNewPhoneNumber_error_text: '',
            ...
            old_number_error: false,
            old_number_error_text: '',
        ...
        try {
            fetch(`${APP_URL}/RoleId2MyProfile`, {
                ...
                await this.setState({
                    ...
                    editName:  response.message.user.name,
                    editSurName:  response.message.user.surname,
                    selectedItemCity: response.message.user.city,
                    selectedItemRegion: response.message.user.region,
                    selectedItemCityID: response.message.user.city_id,
                    selectedItemRegionID: response.message.user.region_id,
                    editOldPhoneNumber: response.message.user.phone,
                    editOldEmail: response.message.user.email,
                    // editOldPassword: response.message.user.password,
                    userPhoto: response.message.user.photo,
                })
    ...
    updateProfile = async () => {
        ...
        let {editName, editSurName, selectedItemRegion, selectedItemRegionID, selectedItemCity, selectedItemCityID} = this.state;

        try {
            fetch(`${APP_URL}/updateRoleId2MyProfile`, {
                method: 'POST',
                headers: {
                    'Authorization': AuthStr,
                    'Accept': 'application/json',
                ...
                body: JSON.stringify({
                    ...
                    region_id: selectedItemRegionID,
                    city: selectedItemCity,
                    city_id: selectedItemCityID,
    ...
    selectImage = async () => {
        ...
        if (!result.cancelled) {
            ...
            if (type == 'jpg' ||  type == 'png' ||  type == 'jpeg') {
                ...
                try {
                    // fetch(`https://steach.justcode.am/api/RoleId2Updateavatar`, {
                    //     method: 'POST',
                    //     headers: {
                    //         'Authorization': AuthStr,
                    //         'Accept': 'application/json',
                    ...
                    let config = {
                        method: 'POST',
                        url: `${APP_URL}/RoleId2Updateavatar`,
                        headers: {
                            Accept: "application/json",
    ...
    setCitiesData = async (selectedItem) => {
        ...
        await this.setState({
            ...
            selectedItemCityID: selected_city_id
        })
    ...
    getRegions = () => {
        try {
            fetch(`${APP_URL}/getRegion`, {
                ...
                await response.message.region.forEach((item) => {
               ...
               await this.getCitiesAfterLoadPage();
    ...
    getCities = async (selectedItem) => {
        ...
        regions_data.forEach((item) => {
            ...
            if(selectedItem == item.name) {
                ...
                this.setState({
                    selectedItemRegionID: region_id,
                });
    ...
    getCitiesAfterLoadPage = async () => {

            let {selectedItemRegionID} = this.state;
            try {
                fetch(`${APP_URL}/getCity/region_id=`+ selectedItemRegionID, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
    ...
    changeEmail = async () => {
        ...
        let {editOldEmail} = this.state;
        let isValidEmail = this.validateEmail(editOldEmail);

        if (isValidEmail === false) {
            this.setState({
                edit_email_error: true,
                edit_email_error_text: 'Не верный формат!'
            })
        } else {
            this.setState({
                edit_email_error: false,
            ...
            try {
                fetch(`${APP_URL}/RoleId2UpdateEmail`, {
                    method: 'POST',
                    headers: {
                        'Authorization': AuthStr,
                        'Accept': 'application/json',
                    ...
                    if (response.hasOwnProperty('email')) {
                        ...
                        let edit_email_error_text ='';
                        if (response.email == 'The email has already been taken.') {
                            edit_email_error_text = 'Электронная почта уже занята.'
                        ...
                        this.setState({
                            edit_email_error: true,
                            edit_email_error_text: edit_email_error_text,
                        })
    ...
    sendOldPhoneNumber = async () => {
        let userToken = await AsyncStorage.getItem('userToken');
        let AuthStr = 'Bearer ' + userToken;

        let {editOldPhoneNumber} = this.state;
        let cleaned_phone_number = editOldPhoneNumber.replace(/\D/g, '');
        if (editOldPhoneNumber.length == 0) {
            this.setState({
                old_number_error: true,
                old_number_error_text: 'Поле является обязательным.'
            })
        } else {
            this.setState({
                old_number_error: false,
            ...
            try {
                fetch(`${APP_URL}/UpdateRoleId2Phone`, {
                    method: 'POST',
                    headers: {
                        'Authorization': AuthStr,
                        'Accept': 'application/json',
                    ...
                    body: JSON.stringify({
                        old_phone: cleaned_phone_number,
                    ...
                    if (response.hasOwnProperty('status')) {
                        if (response.status === true) {
                            this.setState({
                                changeNumberNewPopup: true,
                                changeNumberOldPopup: false,
                            })

                        } else {
                            if (response.message.message == 'wrong number') {
                                this.setState({
                                    old_number_error: true,
                                    old_number_error_text: 'Это неправильный номер'
                                })
                            } else {
                                this.setState({
                                    old_number_error: false,
    ...
    setNewPhoneNumber = async () => {
        let userToken = await AsyncStorage.getItem('userToken');
        let AuthStr = 'Bearer ' + userToken;

        let {editNewPhoneNumber} = this.state;

        let cleaned_phone_number = editNewPhoneNumber.replace(/\D/g, '');



        if (editNewPhoneNumber.length == 0) {
             this.setState({
                 editNewPhoneNumber_error: true,
                 editNewPhoneNumber_error_text: 'Поле «Новый телефон» обязательно для заполнения.'
             })
        } else {
            this.setState({
                editNewPhoneNumber_error: false,
            ...
            try {
                fetch(`${APP_URL}/SendCallNewNumberRoleid2`, {
                    method: 'POST',
                    headers: {
                        'Authorization': AuthStr,
                        'Accept': 'application/json',
                    ...
                    body: JSON.stringify({
                        new_phone: cleaned_phone_number,
    ...
    sendCode = async () => {
        ...
        let {code1, code2, code3, code4, editNewPhoneNumber} = this.state;
        ...
        if (code1.length == 0 || code2.length == 0 || code3.length == 0 || code4.length == 0) {
            ...
            try {
                fetch(`${APP_URL}/CompareCodeNewNumber`, {
                    method: 'POST',
                    headers: {
                        'Authorization': AuthStr,
                        'Accept': 'application/json',
                    ...
                    if (response.hasOwnProperty('status')) {
                        if (response.status === true) {
                            this.setState({
                                ...
                                invalid_code_text: '',
                                changeNumberCodePopup: false,
                                changeNumberSuccessPopup: true

                            })

                        } else {
                            if (response.hasOwnProperty('message')) {
                                 if (response.message.message == 'wrong code') {
                                     this.setState({
                                         phone_code_error: false,
                                         phone_code_error_text: '',
                                         invalid_code: true,
                    ...
                    if (response.hasOwnProperty('phone')) {
                        if (response.phone == 'The phone has already been taken.') {
                            this.setState({
                                ...
                                send_code_again_error_text: 'Телефон уже занят.'
                            })
                        } else {
                            this.setState({
                                send_code_again_error: false,
    ...
    phoneValidation2 = (val) => {
        ...
        const isValid = this.validatePhoneNumber2(myPhone);

        this.setState({
            editOldPhoneNumber: myPhone
        })
    }

    validatePhoneNumber2(phoneNumber) {
        let regex = /^((\7|7|8)+([0-9]){10})$/;
        let new_phone = phoneNumber.replace(/\D/g, '');
        return regex.test(new_phone);
    ...
    render() {
        ...
        //
        // if (this.state.loaded_personal_info) {
        //     return (
        //         <View style={{backgroundColor: '#ffffff', justifyContent: 'center', alignItems: 'center', alignSelf: 'center', width: '100%',  height: windowHeight, position:'absolute', top: 0}}>
        //
        ...
        if (this.state.changeNumberOldPopup) {
            return (

                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={styles.change_number_old_popup_wrapper}>
                        <TouchableOpacity style={styles.change_number_old_popup_close_btn} onPress={() => this.setState({changeNumberOldPopup: false})}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
                        ...
                        <ScrollView style={styles.change_number_old_popup_scroll}>
                            <Text style={styles.change_number_old_popup_info1}>
                                Чтобы изменить номер введите его ниже
                            </Text>
                            <View style={styles.change_number_old_popup_input_field_title_box}>
                                <Text style={styles.change_number_old_popup_input_title}>Старый номер</Text>
                                <TextInput
                                    style={styles.change_number_old_popup_input_field}
                                    // onChangeText={(val) => this.setState({editOldPhoneNumber: val})}
                                    onChangeText={(val) => {
                                        this.phoneValidation2(val)
                                    }}
                                    value={this.state.editOldPhoneNumber}
                                    keyboardType={'phone-pad'}
                                    placeholder='+7ХХХ-ХХХ-ХХ-ХХ'

                                />
                                {this.state.old_number_error &&
                                    <Text style={styles.error_text}>{this.state.old_number_error_text}</Text>
                                }
                            </View>

                            <TouchableOpacity style={styles.change_number_old_popup_confirm_btn} onPress={() => this.sendOldPhoneNumber()}>
        ...
        if (this.state.changeNumberNewPopup) {
            return (

                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={styles.change_number_old_popup_wrapper}>
                        {/*<TouchableOpacity style={styles.change_number_old_popup_close_btn} onPress={() => this.setState({changeNumberNewPopup: false})}>*/}
                        {/*    <Svg*/}
                        {/*        width={30}*/}
                        {/*        height={30}*/}
                        {/*        viewBox="0 0 30 30"*/}
                        ...
                        {/*    >*/}
                        {/*        <Path*/}
                        {/*            d="M4.024 5.351a.938.938 0 111.327-1.327l9.65 9.65 9.648-9.65a.938.938 0 111.328 1.327L16.326 15l9.65 9.649a.939.939 0 01-1.327 1.327L15 16.326l-9.649 9.65a.937.937 0 01-1.602-.663.938.938 0 01.275-.664L13.674 15l-9.65-9.649z"*/}
                        {/*            fill="#888"*/}
                        {/*        />*/}
                        {/*    </Svg>*/}
                        {/*</TouchableOpacity>*/}

                        <TouchableOpacity style={{ position: 'absolute', left: 20, top: 10, zIndex: 999}} onPress={() => this.setState({changeNumberNewPopup: false, changeNumberOldPopup: true })}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
                            ...
                            >
                                <Path
                                    d="M20.169 27.708a1.46 1.46 0 01-1.138-.54l-7.043-8.75a1.458 1.458 0 010-1.851l7.291-8.75a1.46 1.46 0 112.246 1.866L15.006 17.5l6.3 7.817a1.458 1.458 0 01-1.137 2.391z"
                                    fill="#333"
                                />
                            </Svg>
                        </TouchableOpacity>
                        ...
                        <View style={styles.change_number_old_popup_header}>
                            <Text style={[styles.change_number_old_popup_title,  {marginTop: 20}]}>
                                Изменение номера
                            </Text>
                        </View>
                        <ScrollView style={styles.change_number_old_popup_scroll}>
                            ...
                            <View style={styles.change_number_old_popup_input_field_title_box}>
                                <Text style={styles.change_number_old_popup_input_title}>Новый номер</Text>
                                <TextInput
                                    style={styles.change_number_old_popup_input_field}
                                    // onChangeText={(val) => this.setState({editNewPhoneNumber: val})}
                                    onChangeText={(val) => {
                                        this.phoneValidation(val)
                                    }}
                                    value={this.state.editNewPhoneNumber}
                                    placeholder='+7ХХХ-ХХХ-ХХ-ХХ'
                                    // placeholderTextColor='#ffffff'
                                    keyboardType={'phone-pad'}
                                />
                                {this.state.editNewPhoneNumber_error &&
                                     <Text style={styles.error_text}>{this.state.editNewPhoneNumber_error_text}</Text>
                                }
                            </View>



                            <TouchableOpacity style={styles.change_number_old_popup_confirm_btn} onPress={() => this.setNewPhoneNumber()}>
                                <Text style={styles.change_number_old_popup_confirm_btn_text}>Подтвердить</Text>
                            </TouchableOpacity>
                        </ScrollView>
                    </View>
        ...
        if (this.state.changeNumberCodePopup) {
            return (
                ...
                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={styles.change_number_old_popup_wrapper}>
                        ...
                        <TouchableOpacity style={{ position: 'absolute', left: 20, top: 10, zIndex: 999}} onPress={() => this.setState({changeNumberCodePopup: false, changeNumberNewPopup: true })}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
                        ...
                        <ScrollView style={styles.change_number_old_popup_scroll}>
                            <Text style={styles.change_number_old_popup_info2}>
                                На ваш номер отправлен код подтверждения,введите его ниже чтобы подтведить изменение
                            </Text>
                            ...
                            <View style={styles.send_code_again_btn_wrapper}>
                                ...
                                {this.state.phone_code_error &&
                                    <Text style={[styles.error_text, {textAlign: 'center', justifyContent: 'center', alignItems: 'center'}]}>{this.state.phone_code_error_text}</Text>
                                }

                                {this.state.invalid_code &&
                                    <Text style={[styles.error_text, {textAlign: 'center', justifyContent: 'center', alignItems: 'center'}]}>{this.state.invalid_code_text}</Text>
                                }
                            ...
                            <TouchableOpacity style={styles.change_number_old_popup_confirm_btn} onPress={() =>  this.sendCode()}>
                                <Text style={styles.change_number_old_popup_confirm_btn_text}>Подтвердить</Text>
                            </TouchableOpacity>
                        </ScrollView>
                    </View>
        ...
        if (this.state.changeNumberSuccessPopup) {
            return (
                ...
                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={styles.change_number_old_popup_wrapper}>
                        ...
                        <TouchableOpacity style={{ position: 'absolute', right: 20, top: 10, zIndex: 999}} onPress={() => this.setState({changeNumberSuccessPopup: false })}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
        ...
        /* edit mail*/
        if (this.state.changeEmailOldPopup) {
            return (

                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={styles.change_number_old_popup_wrapper}>
                        <TouchableOpacity style={styles.change_number_old_popup_close_btn} onPress={() => this.setState({changeEmailOldPopup: false})}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
                        ...
                        <ScrollView style={styles.change_number_old_popup_scroll}>
                            <Text style={styles.change_number_old_popup_info1}>
                                Введите адрес новой эл. почты
                            </Text>
                            <View style={styles.change_number_old_popup_input_field_title_box}>
                                {/*<Text style={styles.change_number_old_popup_input_title}>Старая эл. почта</Text>*/}
                                <TextInput
                                    style={styles.change_number_old_popup_input_field}
                                    onChangeText={(val) => this.setState({editOldEmail: val})}
                                    value={this.state.editOldEmail}

                                />

                                {this.state.edit_email_error &&
                                    <Text style={styles.error_text}>{this.state.edit_email_error_text}</Text>
                                }
                            </View>

                            <TouchableOpacity style={styles.change_number_old_popup_confirm_btn} onPress={() => this.changeEmail()}>
                                <Text style={styles.change_number_old_popup_confirm_btn_text}>Подтвердить</Text>
                            </TouchableOpacity>
                        </ScrollView>
                    </View>
        ...
        if (this.state.changeEmailSuccessPopup) {
            return (
                ...
                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={styles.change_number_old_popup_wrapper}>
                        ...
                        <TouchableOpacity style={{ position: 'absolute', right: 20, top: 10, zIndex: 999}} onPress={() => this.setState({changeEmailSuccessPopup: false })}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
        ...
        if (this.state.changePasswordPopup) {
            return (
                ...
                <SafeAreaView style={styles.change_number_old_popup}>
                    <View style={[styles.change_number_old_popup_wrapper, {paddingTop: 35}]}>
                        <View style={styles.change_number_old_popup_close_btn_wrapper}>
                            <TouchableOpacity
                        ...
                        <View style={styles.change_number_old_popup_footer}>
            ...
            <SafeAreaView edges={['right', 'left', 'top']} style={styles.container} >
                ...
                <View style={styles.personal_area_header}>
                    <View style={styles.personal_area_header_btn_title_box}>
                        <TouchableOpacity style={styles.personal_area_header_btn} onPress={() => this.redirectToPersonalArea()}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
                ...
                <View style={styles.avatar_img_edit_btn_wrapper}>
                    <View style={styles.avatar_img_wrapper}>
                        <Image style={styles.avatar_img} source={this.state.userPhoto ?  {uri: this.state.image_path + this.state.userPhoto} : require('../../../assets/images/avatar.png')}/>
                ...
                <KeyboardAwareScrollView
                    ...
                    // scrollEnabled={true}
                >
                        <View style={styles.personal_area_input_title_box}>
                            <Text style={styles.personal_area_input_field_name}>Имя</Text>
                            <View style={styles.personal_area_input_field_edit_btn_box}>
                                <TextInput
                                    style={styles.personal_area_input_field}
                                ...
                                {this.state.name_input_disabled
                                    ...
                                    <TouchableOpacity
                                        onPress={() => {
                                            this.makeAbleToWriteName()
                                        }}

                                        style={styles.edit_input_btn}
                                    >
                                        <Svg
                                            width={23}
                                            height={23}
                                        ...
                                        >
                                            <Path
                                                d="M15.788 5.183l2.03 2.028m-.725-3.816l-5.488 5.489a2.03 2.03 0 00-.556 1.037l-.507 2.537 2.538-.508a2.028 2.028 0 001.037-.554l5.488-5.489a1.777 1.777 0 00-2.512-2.512v0z"
                                                stroke="#F60"
                                                strokeWidth={1.2}
                                                strokeLinecap="round"
                                                strokeLinejoin="round"
                                            />
                                            <Path
                                                d="M18.209 14.375v2.875a1.917 1.917 0 01-1.917 1.917H5.75a1.917 1.917 0 01-1.917-1.917V6.708A1.917 1.917 0 015.75 4.792h2.875"
                                                stroke="#F60"
                                                strokeWidth={1.2}
                                                strokeLinecap="round"
                                                strokeLinejoin="round"
                            ...
                            {this.state.edit_name_error &&
                            <Text style={styles.error_text}>{this.state.edit_name_error_text}</Text>
                            }

                        </View>
                        <View style={styles.personal_area_input_title_box}>
                            <Text style={styles.personal_area_input_field_name}>Фамилия</Text>
                            <View style={styles.personal_area_input_field_edit_btn_box}>
                                <TextInput
                                    style={styles.personal_area_input_field}
                                    onChangeText={(val) => this.setState({editSurName: val})}
                                    value={this.state.editSurName}
                                    placeholder={this.state.editSurName}
                                    placeholderTextColor="#B3B3B3"
                                    editable={this.state.surname_input_disabled}
                                />


                                {this.state.surname_input_disabled ?
                                    ...
                                    >
                                        <Svg width={19} height={17} viewBox="0 0 19 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <Path d="M2 10l4.412 5L17 2" stroke="#F60" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"/>
                                        </Svg>
                                    </TouchableOpacity>
                                    ...
                                    <TouchableOpacity
                                        onPress={() => {
                                            this.makeAbleToWriteSurName()
                                        }}
                                      style={styles.edit_input_btn}
                                    >
                                        <Svg width={23} height={23} viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <Path d="M15.788 5.183l2.03 2.028m-.725-3.816l-5.488 5.489a2.03 2.03 0 00-.556 1.037l-.507 2.537 2.538-.508a2.028 2.028 0 001.037-.554l5.488-5.489a1.777 1.777 0 00-2.512-2.512v0z" stroke="#F60" strokeWidth={1.2} strokeLinecap="round" strokeLinejoin="round"/>
                                            <Path d="M18.209 14.375v2.875a1.917 1.917 0 01-1.917 1.917H5.75a1.917 1.917 0 01-1.917-1.917V6.708A1.917 1.917 0 015.75 4.792h2.875" stroke="#F60" strokeWidth={1.2} strokeLinecap="round" strokeLinejoin="round"/>
                                        </Svg>
                                    </TouchableOpacity>
                                }
                            ...
                            {this.state.edit_surname_error &&
                            <Text style={styles.error_text}>{this.state.edit_surname_error_text}</Text>
                            }
                    ...
                    <View style={[styles.sign_up_input_title_wrapper, {position: 'relative', zIndex: 9999999}]}>
                        ...
                        {this.state.selectedItemRegion != '' &&  this.state.regions_data_for_dropdown.length > 0 &&
                            ...
                            <SelectDropdown
                                ...
                                defaultValue={this.state.selectedItemRegion}
                    ...
                    <View style={[styles.sign_up_input_title_wrapper, {position: 'relative', zIndex: 99999}]}>
                        ...
                        {this.state.selectedItemCity != '' && this.state.cities_data_for_dropdown.length > 0 &&
                                ...
                                <SelectDropdown
                                    ...
                                    defaultValue={this.state.selectedItemCity}
                                    buttonTextStyle={{
                                        textAlign:'left',
                                        color: '#B3B3B3',
                                        fontWeight: '400',
                    ...
                    {/*Custom DropDown City END*/}
                        ...
                        <View style={styles.personal_area_input_title_box_number}>
                            <View style={styles.personal_area_number_input_title_btn}>
                                <Text style={styles.personal_area_input_field_name}>Номер</Text>
                                <TouchableOpacity onPress={() => this.setState({changeNumberOldPopup: true})}>
                                    <Svg
                                        width={23}
                                        height={23}
                                        viewBox="0 0 23 23"
                            ...
                            <Text style={styles.personal_area_input_phone_number}>{this.state.editOldPhoneNumber}</Text>


                            {this.state.edit_phone_error &&
                            <Text style={styles.error_text}>{this.state.edit_phone_error_text}</Text>
                            }

                        </View>
                        <View style={styles.personal_area_input_title_box_number}>
                            <View style={styles.personal_area_number_input_title_btn}>
                                <Text style={styles.personal_area_input_field_name}>Эл. почта</Text>
                                <TouchableOpacity onPress={() => this.setState({changeEmailOldPopup: true})}>
                                    <Svg
                                        width={23}
                                        height={23}
                                        viewBox="0 0 23 23"
                            ...
                            <Text style={styles.personal_area_input_phone_number}>{this.state.editOldEmail}</Text>


                            {this.state.edit_mail_error &&
                            <Text style={styles.error_text}>{this.state.edit_mail_error_text}</Text>
                            }
                        ...
                        <View style={styles.personal_area_input_title_box_password}>
                            ...
                            <TouchableOpacity style={styles.personal_area_change_password_btn} onPress={() => this.setState({
                                editOldPassword: '',
                                editNewPassword: '',
                                confirmNewPassword: '',
                        ...
                        <TouchableOpacity style={styles.log_out_btn} onPress={() => {this.updateProfile()}}>
                ...
                {this.state.keyboardOpen === false &&
                    ...
                    <TouchableOpacity style={[styles.footer_btn, {position:'relative', }]} onPress={() => this.redirectToCatalogueMainPage()}>
                        <Svg width={41} height={38} viewBox="0 0 41 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <Path d="M35 28.535l-4.857-4.855a8.314 8.314 0 10-1.464 1.465L33.536 30 35 28.535zm-11.393-3.714a6.214 6.214 0 116.214-6.214 6.221 6.221 0 01-6.214 6.214zM6 11.357h8.286v2.072H6v-2.072zM6 1h16.571v2.071H6V1zm0 5.179h16.571V8.25H6V6.179z" fill="#fff"/>
                        </Svg>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    change_number_old_popup_footer: {
        width: '100%',
        justifyContent: 'center',
        alignItems: 'center',
        height:150
    ...
    change_number_old_popup_close_btn_wrapper: {
      width: '100%',
