...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            initialRegion: {
                latitude:  40.17292365907212,
                longitude: 44.51124502719759,
            ...
            executors_list_loading: false,

            show_executors_list: false,

            category_id: null,
            subcategory_id: null,
            next_page_url: null,
            loading_executors_list: false,
    ...
    //         // loaded_list: true,
    //         show_sub_categories: false,
    //         show_executors_lists: true,
    //         // executors_list_loading: true,
    ...
    //
    //     try {
    //         fetch(`${APP_URL}/FilterUsers?page=1`, {
    //             method: 'POST',
    //             headers: {
    ...
    //             {
    //
    //                 if (response.message.users.hasOwnProperty('data'))
    //                 {
    //                     if (response.message.users.data.length > 0) {
    //                         this.setState({
    //                             executors_list: response.message.users.data,
    //                         })
    //
    //                     }
    //                 }
    ...
    //
    //
    //             if (response.message.users.hasOwnProperty('next_page_url')) {
    //                 if (response.message.users.next_page_url) {
    //                     this.setState({
    //                         next_page_url: response.message.users.next_page_url,
    //                     })
    //                 } else {
    //                     this.setState({
    //                         // loading_executors_list: false,
    ...
    //
    //             }
    //             if (response.message.users.hasOwnProperty('current_page')) {
    //                 if (response.message.users.current_page) {
    //                     this.setState({
    //                         current_page: response.message.users.current_page,
    //                     })
    //                 }
    //             }
    //             if (response.message.users.hasOwnProperty('last_page')) {
    ...
    // }
    //
    // loadMoreExecutorsList = async () => {
    //
    //     let userToken = await AsyncStorage.getItem('userToken');
    //     let AuthStr = 'Bearer ' + userToken;
    //     let {next_page_url, executors_list, subcategory_id, category_id, latitude, longitude, current_page, last_page} = this.state;
    //
    //    await this.setState({
    //         loading_executors_list: true
    //     })
    //
    //
    ...
    //                 if (response.message.hasOwnProperty('users')) {
    //                     if (response.message.users.hasOwnProperty('data')) {
    //                                 if (response.message.users.data) {
    //                                     let new_executors_list = response.message.users.data;
    //                                     new_executors_list.forEach(function(item){
    //                                         executors_list.push(item)
    //                                     })
    //
    //                                 }
    //                         }
    ...
    //     {
    //         Object.assign(req, {sub_category_id: this.state.subcategory_id})
    //     }
    //
    //
    //     try {
    ...
    //             {
    //
    //                 if (response.message.hasOwnProperty('reklam')) {
    //
    ...
    redirectToClientCatalogueSinglePage = (id) => {
        this.props.navigation.navigate("CatalogueProductSinglePage", {
            params: id,
            redirect_from: 'from executors list'
        });
    }


    redirectToProductSingle = () => {
        let {executor_info} = this.state;

        this.setState({
            redirectToSingleProductPopup: false
        })
        this.props.navigation.navigate("CatalogueProductSinglePage", {
            params: executor_info.id
        });
    }
    ...
    getDistance =  (distance) => {
        ...
        let float_distance = parseFloat(distance);

        let final_result = float_distance.toFixed(1);
        ...
        try {
            fetch(`${APP_URL}/getSubcategory/category_id=` + category_id, {
                ...
                if (response.data.length > 0) {
                    console.log(response.data, 'datahgaaaaaaaaaaaaaaaaa')
                    this.setState({
                    ...
                    // get executors list use only category id
                    // show executors list
                   await this.setState({
                        subcategory_id: null,
                        show_categories: false,
    ...
    checkType = (item) => {
        ...
        if (item?.user_transport?.[0]?.user_transport_sub_category?.type) {
            type = item?.user_transport?.[0]?.user_transport_sub_category?.type
            // if (Object.keys(item.user_transport[0].user_transport_sub_category).length == 0) {
            //         type = item.user_transport[0].user_transport_sub_category.type
            // }
        } else if (item?.user_transport?.[0]?.user_transport_category?.type) {

            type = item?.user_transport?.[0]?.user_transport_category?.type;
            // if (Object.keys(item.user_transport[0].user_transport_category).length == 0) {
            //         type = item.user_transport[0].user_transport_category.type
            // }

        }

        return type == 'raice' ? 'рейс' : type == 'time' ? 'час' : '';
   ...
   renderItem  = ({item}) => {
        // let reklam = this.reklam;
        // let reklam_random = null;
        ...
        //
        //                 </View>
        //                 <Text  style={styles.executor_list_item_hour_money_info}>{item.priceJob} руб./час</Text>
        //             </View>
        //
        //
        //         </TouchableOpacity>
       ...
       if (item.reklam)
       {
           ...
           return (
               <View>
                   ...
                   <View  style={[styles.executor_list_item, ]}>
    ...
    // redirectToExecutorsListsWithoutCategory = (item , category_id) => {
    //     let subcategory_id = item.id
    //
    //     console.log(category_id, 'ib bsxbaaaaaaaaaaaaaaaaaaaaaa')
    //
    //     this.props.navigation.navigate("ExecutorsListsWithoutCategory", {
    //         params1: subcategory_id,
    //         params2: category_id,
    //     });
    //
    // }
            ...
            <SafeAreaView  edges={['right', 'left', 'top']} style={styles.container} >
                ...
                >
                    ...
                    <View style={styles.catalogue_list_map_buttons_wrapper}>
                        ...
                        <TouchableOpacity style={[styles.catalogue_list_map_button, this.state.catalogue_map ? styles.activeBackground : {} ]} onPress={() => this.redirectToClientCatalogueMainPage()}>
                ...
                <View style={[styles.catalogue_main_wrapper,]} >
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            this.getSubCategoriesTransports(item)
                                        }}>
                        ...
                        <View style={[styles.catalogue_lists_items_wrapper, ]}>
                            <TouchableOpacity style={{ flexDirection:'row', alignItems: 'center', marginBottom: 20, paddingHorizontal: 20}} onPress={() => {this.setState({show_sub_categories: false, show_categories: true})}}>
                                ...
                                <Text style={{ fontWeight: '500', fontSize: 18, color: '#000000',}}>Назад</Text>
                            </TouchableOpacity>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    <TouchableOpacity
                                        key={item.id}
                                        style={styles.catalogue_lists_item}
                                        // onPress={() => this.getExecutorsList(item)}
                                        onPress={() => {
                                            this.props.navigation.navigate("ExecutorsListsWithoutCategory", {
                                                params1: item.id,
                                                params2: this.state.category_id,
                                            });
                                        }}
                                    >
                                ...
                                Setting the number of column
                                numColumns={3}
                                columnWrapperStyle={{flex: 1, justifyContent: "flex-start", paddingHorizontal: 20}}
                                keyExtractor={(item, index) => index.toString()}
                        ...
                        {/*{ this.state.show_executors_lists &&*/}
                        {/*    <View style={{backgroundColor: '#ffffff', justifyContent: 'center', alignItems: 'center', alignSelf: 'center', width: '100%',  height: windowHeight -200, position:'absolute', top: this.state.catalog_header_height -200 }}>*/}
                        ...
                        {/*{ this.state.show_executors_lists && !this.state.show_sub_categories && !this.state.show_categories &&*/}

                        {/*    <View style={[styles.executors_lists_wrapper, {paddingHorizontal: 9, position: 'relative'}]}>*/}
                        {/*        {this.state.executors_list.length > 0 &&*/}
                        {/*            <View>*/}


                        {/*            <FlatList*/}
                        {/*                data={this.state.executors_list}*/}
                        {/*                nestedScrollEnabled*/}
                        {/*                renderItem={this.renderItem}*/}
                        {/*                //Setting the number of column*/}
                        {/*                keyExtractor={(item, index) => index.toString()}*/}
                        {/*                onEndReached={() => this.loadMoreExecutorsList()}*/}
                        {/*                onEndReachedThreshold={0.5}*/}
                        {/*            />*/}

                        {/*            {this.state.loading_executors_list &&*/}
                        {/*            <View style={{*/}
                        ...
                        {/*        {!this.state.loader && this.state.executors_list.length == 0 &&*/}
...
const styles = StyleSheet.create({
    ...
    executors_lists_wrapper: {
        width: '100%',
        flex: 1,
        // backgroundColor: 'red'

    },
    executor_list_item_img: {
        width: 40,
        height: 40,
        marginRight: 8,
    },

    executor_list_item_img_child: {
        width: '100%',
        height: '100%',
        resizeMode: 'cover',
        borderRadius: 100,
    },
    executor_list_item_img_name_wrapper: {
        flexDirection: 'row',
        alignItems: 'center',
        width: '70%',
    },
    executor_list_item_distance_info: {
        fontWeight: '300',
        fontSize: 16,
        color: '#000000',
        marginLeft: 5,
    },
    executor_list_item_distance_info_box: {
        flexDirection: 'row',
        alignItems: 'center',
    },
    executor_list_item_distance_hour_money_info: {
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'space-between',
        width: '100%',
    },
    executor_list_item_hour_money_info: {
        color: '#FF6600',
        fontWeight: '400',
        fontSize: 20,
    },
    executor_list_item_name: {
        color: '#333333',
        fontSize: 16,
        fontWeight: '400',
    ...
    executor_list_item_rating_info_box: {
        borderRadius: 6,
        backgroundColor: '#FF6600',
        width: 45,
        height: 32,
    ...
    executor_list_item_rating_info: {
        fontWeight: '400',
        fontSize: 17,
        color: '#ffffff',
    },
    executor_list_item_img_name_rating_info_wrapper: {
        width: '100%',
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'space-between',
