...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            password_does_not_match_error: false,
            password_does_not_match_error_text: '',
    ...
    selectImage = async () => {
        ...
        if (!result.cancelled) {
            ...
            if (type == 'jpg' ||  type == 'png' ||  type == 'jpeg') {
                ...
                fetch(
                    'http://80.78.246.59/BandRate-Smart/public/api/updatephoto',
                    {
                        body: form,
                        method: "POST",
                        headers: {
    ...
    sendPhoneAgain = () => {
        ...
        try {
            fetch(`${APP_URL}/dublnewPhoneRegister`, {
                ...
                if (response.status=== true) {
                    if (response.hasOwnProperty('message')) {
                         if (response.message.message == 'code send yor phone') {
                             this.setState({
                                 invalid_code: false,
                                 invalid_code_text: '',
    ...
    signUpProfile = () => {
        let {name, surname, email, password, confirmPassword,selectedItemCity, selectedItemRegion, user_image, phone, region_id, selected_city_id} = this.state;

        if (name.length == 0 || surname.length == 0 || email.length == 0 || password.length == 0 || confirmPassword.length == 0 || selectedItemCity.length == 0 || selectedItemRegion.length == 0 || user_image.length == 0  ) {
            if (name.length == 0) {
                this.setState({
                    name_error: true,
            ...
            if (password.length == 0) {
                this.setState({
                    ...
                    password_does_not_match_error_text: '',
                    password_error: true,
                    password_error_text: 'Поле является обязательным.',
                })
            } else {
                this.setState({
                    password_error: false,
            ...
            //         this.setState({
            //             password_does_not_match_error: true,
            //             password_does_not_match_error_text: 'Пароли не совпадают',
            //         })
            // } else {
            //     this.setState({
            //         password_does_not_match_error: false,
                ...
                fetch(`${APP_URL}/register`, {
                    ...
                    if (response.status === true) {
                        ...
                        if (response.data.hasOwnProperty('password_confirmation')) {
                            ...
                            if (response.data.password_confirmation[0] == 'The password confirmation and password must match.') {
                            ...
                            this.setState({
                                password_does_not_match_error: true,
                                password_does_not_match_error_text: password_confirmation_error_text,
                                name_error: false,
                                name_error_text: '',
                                surname_error: false,
                                surname_error_text: '',
    ...
    render() {
        ...
        if (this.state.successFinalPopup) {
            return (
                <ImageBackground resizeMode="cover" source={require('../../../assets/images/popup_img.png')} style={[styles.successPasswordPopup,{  zIndex: 99999999, }]}>


                    <View style={styles.successPasswordPopup_wrapper}>

                        <TouchableOpacity style={styles.successPasswordPopup_close_btn} onPress={() => this.setState({successFinalPopup: false})}>
                    ...
                    <View style={styles.sign_up_as_executor_confirmation_phone_popup_wrapper}>
                        <View style={styles.recovery_account_code_header}>
                            <View style={styles.back_to_recovery_email_btn_wrapper}>
                                <TouchableOpacity style={styles.back_recovery_email_btn}  onPress={() => this.setState({
                                    code: ['', '', '', '',],
                                    phone_code_error: false,
                                    phone_code_error_text: '',
                                    invalid_code: false,
                            ...
                            <View style={styles.sign_up_input_title_wrapper}>
                                <View style={styles.sign_up_input_title_error_text_wrapper}>
                                    ...
                                    <View style={{flexDirection: 'column', alignItems: 'flex-end'}}>
                                        ...
                                        {this.state.password_does_not_match_error &&
                                        <Text style={[styles.error_text, {marginTop: 0}]}>{this.state.password_does_not_match_error_text}</Text>
                                        }
                                    </View>
