...
export default class App extends Component {
    ...
    async checkPopupStatus() {
        let receiver_id = this.props.receiver_id;

        try {
            const itemKey = `popupShownTender_${receiver_id}`;
            const popupShown = await AsyncStorage.getItem(itemKey);

            console.log(receiver_id, 'user --id')
    ...
    redirectToClientCatalogueSinglePage = () => {
        this.props.navigation.navigate("CatalogueProductSinglePage", {
            params: this.props.receiver_id,
            redirect_from: 'from chat tender single page',
    ...
    getAllMessages = async () => {
        ...
        let receiver_id = this.props.receiver_id;
        let tender_id = this.props.tender_id;


        try {
            fetch(`${APP_URL}/singlePageChatTender`, {
                method: 'POST',
                headers: {
                ...
                if (response.status === true && response.hasOwnProperty("invite")) {
                    this.setState({
                ...
                if (response.status === true && response.data.single_chat.length > 0) {
                    this.setState({
                        all_messages: response.data.single_chat,
                    })
    ...
    createMessage = async () => {
        ...
        let tender_id = this.props.tender_id;
            ...
            try {
                fetch(`${APP_URL}/createNewMessageTender`, {
                    method: 'POST',
                    headers: {
                        'Authorization': AuthStr,
                        'Accept': 'application/json',
    ...
    selectImage = async () => {
        ...
        if (!result.cancelled) {
            ...
            form.append("tender_id", tender_id);
            form.append("message", '');
            ...
            if (type == 'jpg' ||  type == 'png' ||  type == 'jpeg') {
                ...
                try {
                    ...
                    let config = {
                        method: 'POST',
                        url: `${APP_URL}/createNewMessageTender`,
                        headers: {
                            Accept: "application/json",
                    ...
                    axios(config).then(async (response) => {
                        ...
                        if (response.status === true) {
                            ...
                            if (response.message == 'Подождите пока заказчики ответит вам') {
                                await this.setState({
                                    messageInput: '',
                                    loading_message: false,
                                    wait_for_client_answer_note: true,
                                })

                                setTimeout(async () => {
                                    await this.setState({
                                        wait_for_client_answer_note: false,
                                    })
                                }, 3000)
    ...
    redirectToClientTendersChat = () => {
        clearInterval(this.interval)
        this.props.navigation.navigate("ClientTendersChat");
    }
            ...
            try {
                fetch(`${APP_URL}/addNewInvite`, {
                    ...
                    if (response.status === true) {
                        ...
                        this.setState({
                            user_has_active_job: true,
                            user_has_active_job_text: 'У пользователя есть активная работа',
                            district_error: false,
                            district_error_text: '',
                            city_error: false,
                            city_error_text: '',
            ...
            return (
                <ImageBackground resizeMode="cover" source={require('../../../assets/images/popup_img.png')} style={[styles.redirectToSingleProductPopup,{width: '100%', height: windowHeight,  zIndex: 99999999, }]}>
                    <View style={styles.single_product_popup_wrapper}>
                        ...
                        <TouchableOpacity style={styles.single_product_popup_close_btn} onPress={() => {this.setState({confirmSelectionPerformerPopup: false, user_has_active_job: false, user_has_active_job_text: ''})}}>
                            <Svg
                                width={30}
                                height={30}
                                viewBox="0 0 30 30"
                        ...
                        <Text style={[styles.error_text, {marginTop: 0, marginBottom: 10, fontSize: 14}]}>{this.state.user_has_active_job_text}</Text>
                        }
        ...
        return (
            <SafeAreaView  edges={['right', 'left', 'top']} style={styles.container} >
                ...
                <View style={{ overflow: 'hidden', paddingBottom: 5, width: '100%' }}>
                    ...
                    >
                        ...
                        <View style={styles.client_chat_header}>
                            ...
                            <TouchableOpacity style={styles.client_chat_header_img} onPress={() => {this.redirectToClientCatalogueSinglePage()}}>
                                <Image style={styles.client_chat_header_img_child} source={{uri: this.state.image_path + this.state.user_image}}/>
                            </TouchableOpacity>
                        </View>
                ...
                >
                    ...
                    {/*<View style={styles.client_chat_hour_info_line_box}>*/}
                    {/*    <Text style={styles.client_chat_hour_info}>01.08.22</Text>*/}
                    {/*    <View style={styles.client_chat_hour_info_line}></View>*/}
                    {/*</View>*/}
                                ...
                                {this.state.user_id == chat_item.receiver_id ?
                                    <View style={{alignItems: 'flex-start', width: '100%'}}>
                                        ...
                                        <View  style={styles.client_chat_messages_item_get}>
                                            <Text  style={styles.client_chat_message}>{chat_item.messages}</Text>
                                            <Svg
                                                style={{position: 'absolute', bottom: -1, left: -5   , zIndex: -5,  transform: [{ rotate: '10deg'}], }} xmlns="http://www.w3.org/2000/svg" width={20} height={16} viewBox="0 0 14 16" fill="none">
                                                <Path d="M6.087 0c.106 5.478-.23 9.029-1.011 10.652-.725 1.507-2.265 2.963-4.619 4.37l-.359.21a.191.191 0 00-.07.266.2.2 0 00.153.095c4.974.47 9.058-.667 12.25-3.412L13.014 0H6.087z" fill="#DCDCDC"/>
                                            </Svg>
                                        </View>
                                        }

                                        {chat_item.file &&
                                         <TouchableOpacity  style={[styles.client_chat_messages_item_get_img, {width: 130}]}
                                                            onPress={() => {this.openImage( this.state.image_path + chat_item.file)} }
                                         >
                                            <Image  style={{width: '100%', height: 63, resizeMode: 'cover', borderRadius: 10, overflow: 'hidden'}}  source={{uri: this.state.image_path + chat_item.file}}/>
                                            <Svg
                                    ...
                                    <View style={{alignItems: 'flex-end', width: '100%'}}>
                                        ...
                                        <View  style={styles.client_chat_messages_item_send}>
                                            <Text  style={[styles.client_chat_message, {color: '#ffffff'}]}>{chat_item.messages}</Text>
                                        ...
                                        {chat_item.file &&
                                        <TouchableOpacity  style={[styles.client_chat_messages_item_send_img, {width: 130}]}
                    ...
                    {/*<TouchableOpacity  style={[styles.client_chat_messages_item_send, {flexDirection: 'row',alignItems: 'center' }]}>*/}
                    {/*    <View style={styles.client_chat_messages_item_img}>*/}
                    {/*        <Image style={styles.client_chat_messages_item_img_child}  source= {require('../../../assets/images/chat_message_img.png')} />*/}
                    {/*    </View>*/}
                    {/*    <Text  style={styles.client_chat_message}>Пример.Png</Text>*/}
                    {/*</TouchableOpacity>*/}
                ...
                {this.state.keyboardOpen === false &&
                    ...
                    <TouchableOpacity style={[styles.footer_btn, {position:'relative', }]} onPress={() => {this.redirectToClientCatalogueMainPage()}}>
                    ...
                    <TouchableOpacity style={[styles.footer_btn, {position:'relative', }]} onPress={() => this.redirectToClientPersonalArea()}>
                ...
                <View style={styles.client_answer_popup}>
                    <View style={styles.chat_img_popup_wrapper}>
                        <TouchableOpacity style={{position: 'absolute', right: 10, top: 10, zIndex: 9}}
...
const styles = StyleSheet.create({
    ...
    show_important_message_box: {

    },
    show_important_message_box_text: {
    ...
    client_chat_messages_item_send_img: {
        paddingHorizontal: 5,
        paddingVertical: 5,
        alignSelf: 'flex-end',
        // backgroundColor: '#DEDEDE',
    ...
    client_chat_messages_item_get_img: {
        paddingHorizontal: 5,
        paddingVertical: 5,
        alignItems: 'flex-start',
        justifyContent: 'flex-start',
