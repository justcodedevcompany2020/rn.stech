...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            workDescription_error: false,
            workDescription_error_text: '',
            tenderName_error: false,
            tenderName_error_text: '',

            technology_images_error: false,
            technology_images_error_text: '',
            ...
            wrong_image_error: false,
            wrong_image_error_text: '',
        ...
        if (!result.cancelled) {
            ...
            if (type == 'jpg' ||  type == 'png' ||  type == 'jpeg') {
                ...
                this.setState({
                    technology_images_error: false,
                    technology_images_error_text: '',
                    wrong_image_error: true,
                    wrong_image_error_text: 'Не верный формат картинки!',
                })
            }
    ...
    onChangeDate = async (event, timeOriginValue) => {
    ...
    myJobs = () => {
        this.props.navigation.navigate('MyJobTender');
    }
    ...
    makeAnTender = async () => {
        ...
        let sub_category_id = this.props.sub_category_id;
        let sub_category_name = this.props.sub_category_name;

        let {region_id,selected_city_id,selectedItemRegion, selectedItemCity, street, startCompletedDate, workDescription, tenderName, technology_images} = this.state;

        if (selectedItemRegion.length == 0 || selectedItemCity.length == 0 || street.length == 0 || startCompletedDate.length == 0 || workDescription.length == 0 || tenderName.length == 0 || technology_images[0] == 'plus') {

                if (selectedItemRegion == 'Выберите область') {
                     this.setState({
                         selectedItemRegion_error: true,
                         selectedItemRegion_error_text: 'Поле обязательно'
                     })
                } else {
                    this.setState({
                        selectedItemRegion_error: false,
            ...
            if (selectedItemCity == 'Выберите город') {
                this.setState({
                    ...
                    selectedItemCity_error_text: 'Поле обязательно',
                })
            } else {
                this.setState({
                    selectedItemCity_error: false,
            ...
            if (street.length == 0) {
                this.setState({
                    street_error: true,
                    street_error_text: 'Поле обязательно',
                })
            } else {
                this.setState({
                    street_error: false,
            ...
            if (startCompletedDate.length == 0) {
                this.setState({
                    time_error: true,
                    time_error_text: 'Поле обязательно',
                })
            } else {
                this.setState({
                    time_error: false,
            ...
            if (workDescription.length == 0) {
                this.setState({
                    workDescription_error: true,
                    workDescription_error_text: 'Поле обязательно',
                })
            } else {
                this.setState({
                    workDescription_error: false,
            ...
            if (tenderName.length == 0) {
                this.setState({
                    tenderName_error: true,
                    tenderName_error_text: 'Поле обязательно',
                })
            } else {
                this.setState({
                    tenderName_error: false,
            ...
            if (technology_images[0] == 'plus') {
                this.setState({
                    wrong_image_error: false,
                    wrong_image_error_text: '',
                    technology_images_error: true,
                    technology_images_error_text: 'Поле обязательно',
                })
            } else {
                this.setState({
                    wrong_image_error: false,
            ...
            form.append("sub_category_name", sub_category_name );
            form.append("region_id", region_id );
            form.append("region_name", selectedItemRegion);
            form.append("city_id", selected_city_id);
            form.append("city_name", selectedItemCity);
            form.append("street", street);
            form.append("date_time", startCompletedDate);
            form.append("description", workDescription);
            form.append("name", tenderName);
            technology_images.forEach((item, index) => {
                if (item != 'plus') {
                    let photo = {
                        uri: item,
                        type: 'image/jpeg',
            ...
            try {
                // fetch(`https://steach.justcode.am/api/CreateTender`, {
                //     method: 'POST',
                //     headers: {
                ...
                axios(config).then( async (response) => {
                    ...
                    if (response_data.status == true) {
                        ...
                        // if (response_data.data.hasOwnProperty('description')) {
                        //     if (response_data.data.description[0] == 'The description must be at least 30 characters.') {
                        //         this.setState({
                        //             workDescription_error: true,
                        //             workDescription_error_text: 'Описание должно содержать не менее 30 символов.',
                        //         })
                        //     } else {
                        //         this.setState({
                        //             workDescription_error: false,
                        ...
                        if (response_data.data.hasOwnProperty('name')) {
                             if (response_data.data.name[0] == 'The name must not be greater than 20 characters.') {
                                    this.setState({
                                        tenderName_error: true,
                                        tenderName_error_text: 'Имя не должно быть длиннее 20 символов.',
                                    })
                             } else {
                                 this.setState({
                                     tenderName_error: false,
    ...
    render() {
        ...
        if (this.state.orderModerationPopup) {
            return (
                <ImageBackground resizeMode="cover" source={require('../../../assets/images/popup_img.png')} style={[styles.orderModerationPopup,{width: '100%', height: windowHeight,  zIndex: 99999999, }]}>
                    <View style={styles.order_moderation_popup_wrapper}>
                        <TouchableOpacity style={styles.single_product_popup_close_btn} onPress={() => {this.setState({
                            orderModerationPopup: false,
                            tenderName: '',
                            selectedItemRegion: 'Выберите область',
                            selectedItemCity: 'Выберите город',
                            street: '',
                            startCompletedDate: 'дд.мм.гг',
                            workDescription: '',
                        ...
                        <Text style={styles.single_product_popup_title2}>
                            Просим вас ожидать окончания
                            проверки
                        </Text>

                        <TouchableOpacity style={styles.go_to_single_product_btn} onPress={() => this.myJobs()}>
                            <Text style={styles.go_to_single_product_btn_text}>Мои заказы</Text>
                        </TouchableOpacity>
        ...
        return (
            <SafeAreaView  edges={['right', 'left', 'top']} style={styles.container} >
                    ...
                    <View style={styles.catalogue_header}>
                        ...
                        <TouchableOpacity style={styles.catalogue_header_back_btn} onPress={() => {this.redirectToCreateOrderCategory()}}>
                            <Svg
                                width={35}
                                height={35}
                                viewBox="0 0 35 35"
                ...
                <KeyboardAwareScrollView
                    style={styles.create_an_order_form_wrapper}
                ...
                >
                    ...
                    <View style={styles.create_an_order_form_input_title_wrapper}>
                        <Text style={styles.create_an_order_form_input_title}>
                            Название тендера
                        </Text>

                        <TextInput
                            style={styles.create_an_order_form_input_field}
                            onChangeText={(val) => this.setState({tenderName: val})}
                            value={this.state.tenderName}
                        />

                        {this.state.tenderName_error &&
                            <Text style={styles.error_text}>{this.state.tenderName_error_text}</Text>
                        }
                    ...
                    <View style={[styles.sign_up_input_title_wrapper, {position: 'relative', zIndex: 9999999}]}>
                        ...
                        <SelectDropdown
                            ...
                            buttonStyle={{width: '100%', backgroundColor: '#F1F1F1', borderRadius: 6,}}
                            buttonTextStyle={{ textAlign:'left', fontSize: 15, fontWeight: '400', color: '#000000'}}
                        ...
                        {this.state.selectedItemRegion_error &&
                          <Text style={[styles.error_text,]}>{this.state.selectedItemRegion_error_text}</Text>
                        }
                    ...
                    <View style={styles.create_an_order_form_input_title_wrapper}>
                        ...
                        >
                            ...
                            <Text
                               style={{width: '100%',fontWeight: '400', fontSize: 15, color: '#000000',}}
                            >
                        ...
                        {this.state.time_error &&
                          <Text style={styles.error_text}>{this.state.time_error_text}</Text>
                        }
                    ...
                    <View style={styles.create_an_order_form_input_title_wrapper}>
                        ...
                        <TextInput
                            style={styles.work_description_input_field}
                            onChangeText={(val) => this.setState({workDescription: val})}
                            value={this.state.workDescription}
                            multiline={true}
                        />

                        {/*{this.state.workDescription_error &&*/}
                        {/*     <Text style={styles.error_text}>{this.state.workDescription_error_text}</Text>*/}
                        {/*}*/}

                    </View>

                    <View style={styles.photos_of_technology_wrapper}>
                        <Text style={styles.sign_up_img_title}>Фотографии</Text>
                        ...
                        {this.state.technology_images_error &&
                        <Text style={styles.error_text}>{this.state.technology_images_error_text}</Text>
                        }
                        {this.state.wrong_image_error &&
                            <Text style={styles.error_text}>{this.state.wrong_image_error_text}</Text>
                        }

                    </View>

                    <TouchableOpacity style={styles.create_an_order_btn}  onPress={() => this.makeAnTender()} >
                        <Text style={styles.create_an_order_btn_text}>Создать</Text>
                    </TouchableOpacity>
                ...
                {this.state.isOpenStartDatePicker &&
                    ...
                    <View style={styles.timepicker_popup_wrapper}>
                        <TouchableOpacity style={{width: '100%', position: 'absolute', left: 20, top: 60}} onPress={() => {this.setState({isOpenStartDatePicker: false})}}>
                            <View>
                                <Svg
                                    width={12}
                                    height={20}
...
const styles = StyleSheet.create({
    ...
    create_an_order_form_input_title_wrapper: {
        marginBottom: 15,
    },
    create_an_order_form_input_title: {
        fontWeight: '500',
        fontSize: 15,
        color: '#000000',
        marginBottom: 12,
    },
    create_an_order_form_input_field: {
        width: '100%',
    ...
    create_an_order_btn_text: {
        color: '#ffffff',
        fontSize: 18,
        fontWeight: '700',
    },
    order_moderation_popup_wrapper: {
        position: 'relative',
        paddingTop: 78,
        paddingBottom: 59,
